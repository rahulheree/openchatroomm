...
const getSessionToken = () => apiClient.get("/session/token");

const createRoom = (roomData) => apiClient.post("/rooms", roomData);
const getCommunityRooms = () => apiClient.get("/rooms/community");
const getUserspaceRooms = () => apiClient.get(`/rooms/userspaces?t=${Date.now()}`);
...
function App() {
    ...
    }, [selectedRoom, user]); // Removed myRooms to prevent re-render loop if myRooms changes (unread count)
